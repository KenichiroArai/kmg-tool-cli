...
3. **ツールの実行**
   ...
   ```bash
   # Mavenを使用して実行
   mvn exec:java -Dexec.mainClass="kmg.tool.cli.msgtpcrt.presentation.ul.cli.MessageTypesCreationTool"

   # または、JARファイルから実行
   java -cp target/classes:target/dependency/* kmg.tool.cli.msgtpcrt.presentation.ul.cli.MessageTypesCreationTool
   ```
