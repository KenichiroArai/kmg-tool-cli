...
3. **ツールの実行**
   ...
   ```bash
   # Mavenを使用して実行
   mvn exec:java -Dexec.mainClass="kmg.tool.cli.simple.presentation.ui.cli.SimpleTwo2OneTool"

   # または、JARファイルから実行
   java -cp target/classes:target/dependency/* kmg.tool.cli.simple.presentation.ui.cli.SimpleTwo2OneTool
   ```
